...
function VerticalPaneSlotComponent({ slotId, uiState, uiController, settings, onHome }: SlotProps) {
    ...
    if (!slotState?.contentType) {
        return (
            <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#1c1f26', color: '#9ca3af', fontSize: '12px', fontFamily: "'IBM Plex Mono', monospace" }}>
                No content loaded
            </div>
        )
    }

...
function PDFContentComponent({ 
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      <PageIndicator
        slotId={slotId}
        pageInput={slotState?.pageInput || ''}
        currentPage={slotState?.currentPage}
        numPages={numPages}
        onPageInputChange={(value) => uiController.setSlotState(slotId, 'pageInput', value)}
      ...
      <VerticalToolBar
        settings={settings}
        slotState={slotState}
        toolRenderers={Array.from(toolRendererRegistry.values())
        ...
        setMarksWithSectionWidths={setMarksWithSectionWidths}
        handleCreateFromPanel={handleCreateFromPanel}
        actions={{
          setTool: (tool) => uiController.setSlotState(slotId, 'tool', tool),
          setSectionTarget: (target) => uiController.setSlotState(slotId, 'sectionTarget', target),
          setEditingSectionId: (id) => uiController.setSlotState(slotId, 'editingSectionId', id),
          ...
          setShapeBackup: (backup) => uiController.setSlotState(slotId, 'shapeBackup', backup),
          setEditingShapeId: (shapeId) => uiController.setSlotState(slotId, 'editingShapeId', shapeId),
          setZoom: (zoom) => uiController.setSlotState(slotId, 'zoom', zoom),
        }}
      />
    </div>
  );
